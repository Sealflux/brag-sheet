# Brag Sheet — Thuan Nguyen

*"The secret of success is… to be fully awake to everything about you." - Jackson Pollock*

---

## Quick Summary

**Core Skills:** Roblox Lua, React + TypeScript, PCB Design (KiCad), Embedded Systems (ESP32/RP2040), 3D Printing, Systems Architecture, UI/UX Design, Project Management, Fabrication, Data Stores

**Key Projects:** Desert Royale (web game), DeepThought (keychain device), NFC Business Card, Friendship Signal Bracelets, RP2040 Dev Board

**Interests:** Software Engineering, Embedded Systems, Hardware Design, Game Design, Product Management


---

## Annual Goals — 2025-2026 (Junior Year)

**Theme:** "Survive Junior Year while studying for 6 AP Exams"

| Goal | Result |
|------|--------|
| Score 4+ on all AP Exams | Mixed — see below |
| Ship one monetized Roblox game | In progress |
| Lead one major initiative in a club | |
| Read 12 non-fiction books | |


---

## Academics & Test Scores

| Exam | Date | Score | Notes |
|------|------|-------|-------|
| AP United States History | May 2024 | 5 | |
| AP Physics 1 | May 2024 | 3 | |
| PSAT/NMSQT | Oct 2024 | 1420 (99th %) | One practice test |
| AP Gov | May 2025 | 5 | |
| AP English Seminar | May 2025 | 3 | |
| SAT | June 2025 | 1330 (89th %) | English dragged me down |
| AP World | May 2026 | 5 | |
| AP Lang | May 2026 | 4 | |
| AP Physics C: Mechanics | May 2026 | 3 | |
| AP Calc BC | May 2026 | 3 | |
| AP Bio | May 2026 | 3 | Self-studied, no school class |
| AP Physics C: E&M | May 2026 | 2 | |
| SAT | March 2026 | 1390 | |
| SAT | August 2026 | TBD | Currently studying for 1500+ |

**Coursework Highlights:** *(list challenging/advanced classes)*


---

## Project Deep Dives

### Desert Royale
**Timeframe:** 2025–Present | **Status:** Shipped (Live)  
**Repo:** [https://github.com/Sealflux/Desert-Royale](https://github.com/Sealflux/Desert-Royale)

Turn-based survival board game set in the wild west. 2-6 players fight to be the last one standing in a grid-based arena. Digital adaptation of an original board game inspired by Guncho.

**Tech Stack:** React, TypeScript, Boardgame.io, Tailwind CSS, Vite

**Role:** Solo Developer

**Key Achievements:**
- Implemented 26 action cards across 4 types (Movement, Combat, Utility, Bullet)
- Built full game logic with turn-based mechanics and action point system
- Deployed live on Vercel

**Future Plans:** UI/UX overhaul, mobile support, multiplayer support, code refactoring


### DeepThought
**Timeframe:** 2025–Present | **Status:** Prototype (pending funding)  
**Repo:** [https://github.com/Sealflux/DeepThought](https://github.com/Sealflux/DeepThought)

Pocket keychain device that displays DeepSeek quotes on an OLED at the push of a button.

**Hardware:** Seeed XIAO ESP32-C3, SSD1306 OLED, tactile button, 250mAh LiPo battery, 3D-printed case

**Role:** Solo Designer/Engineer

**Key Achievements:**
- Designed custom PCB in KiCad
- 3D modeled enclosure
- Wrote Arduino firmware for display and button logic


### NFC Business Card
**Timeframe:** 2025–Present | **Status:** Designed (pending funding)  
**Repo:** [https://github.com/Sealflux/PCB_Business_Card](https://github.com/Sealflux/PCB_Business_Card)

NFC business card that lights up when tapped — no battery, harvests power from phone's NFC field.

**Key Achievements:**
- Used NXP NT3H1101 NFC chip
- Energy harvesting circuit with 220nF capacitor
- 4 LEDs light up on tap
- Customizable — edit copper layers/mask for text/logo

**Role:** Solo Designer/Engineer


### Friendship Signal Bracelets
**Timeframe:** 2025–Present | **Status:** In Development  
**Repo:** [https://github.com/Sealflux/Friendship-Signal-Bracelets](https://github.com/Sealflux/Friendship-Signal-Bracelets)

Bracelets that light up and vibrate when besties are within ~3-5 meters using BLE RSSI proximity detection.

**Hardware:** Seeed XIAO ESP32-C3, OLED, 6x WS2812B LEDs, microphone, accelerometer, buzzer, LiPo battery

**Role:** Solo Designer/Engineer


### RP2040 Dev Board
**Timeframe:** 2025–Present | **Status:** Designed  
**Repo:** [https://github.com/Sealflux/RP2040Devboard](https://github.com/Sealflux/RP2040Devboard)

Custom dev board built around the Raspberry Pi RP2040 MCU.

**Key Achievements:**
- Designed full PCB with RP2040, 16MB flash, USB-C, crystal oscillator
- Learned the tedious details of MCU board design

**Role:** Solo Designer/Engineer


---

## Leadership & Extracurriculars

### Robotics Team — Mechanical Team Member (2023–2024)
- Fabricated components from CAD drawings using bandsaws and drill presses
- Collaborated with design and software teams to build high-tolerance assemblies

### Science Bowl — Member (2023–Present)
- Specialized in Chemistry; 5+ hours/week independent study
- Competed in 2+ tournaments as part of 4-person team

### Science Olympiad — Member (2023–Present)
- Events: Anatomy & Physiology, Chem Lab, Material Science, Experimental Design, Botany, Remote Sensing, Machines
- Made it to Maryland State Tournament

**Awards:**
- 2/35 — Remote Sensing (Regional 2026)
- 3rd Place — Machines (UMBC Invitational 2026)
- 4/42 — Botany (State 2025)
- 6/25 — Chem Lab (UMD Invitational 2025)
- 7/42 — Anatomy & Physiology (Regional 2025)
- 7/24 — Anatomy & Physiology (State 2025)
- 9/24 — Material Science (State 2025)
- 11/25 — Anatomy & Physiology (UMD Invitational 2025)
- 12/42 — Chem Lab (Regional 2025)
- 13/24 — Experimental Design (State 2025)
- 14/25 — Material Science (UMD Invitational 2025)
- 19/42 — Material Science (Regional 2025)

Developed rigorous testing protocols for device optimization, iterating on designs based on empirical results to maximize competition scores.

### Kumon Inc. — Teaching Assistant (Oct 2024 – Feb 2026)
- Managed personalized learning plans for 100+ K-12 students
- Handled data entry and grading with 100% accuracy
- Mentored and onboarded 1 new teaching assistant
- Used student performance data to recommend lesson adjustments

### National Honor Society — Member (2025–Present)
- Received Outstanding Mentoring Award (March 25, 2026) for peer tutoring

### School Tutoring (2023–Present)
- Tutored multiple students across several subjects since sophomore year


---

## Awards & Recognition

| Award | Level | Year |
|-------|-------|------|
| Outstanding Mentorship Certificate | School | 2026 |
| 2nd Place — Remote Sensing (Science Olympiad) | Regional | 2026 |
| 3rd Place — Machines (Science Olympiad UMBC Invitational) | Invitational | 2026 |
| 4th Place — Botany (Science Olympiad) | State | 2025 |
| 7th Place — Anatomy & Physiology (Science Olympiad) | Regional | 2025 |
| 7th Place — Anatomy & Physiology (Science Olympiad) | State | 2025 |


---

## Skill Progression

| Skill | Beginner | Novice | Intermediate | Advanced | Notes |
|-------|----------|--------|--------------|----------|-------|
| Roblox Lua | X | | | X | Confident with core API |
| React + TypeScript | | | X | | Desert Royale |
| PCB Design (KiCad) | | X | | | 4+ board designs |
| Embedded Systems | | X | | | ESP32, RP2040 |
| 3D Printing | | X | | | Case design |
| 3D Modeling (Onshape) | | X | | | CAD for enclosures |
| 3D Modeling (Fusion 360) | X | | | | Learning workflow |
| Client-Server | X | | | X | Understands security |
| Data Stores | X | | | X | ProfileService |
| UI/UX Design | X | | | | Needs work on responsive |
| Git / Version Control | | X | | | All projects on GitHub |
| Project Management | | X | | | Time-blocking |
| Public Speaking | | | X | | Comfortable leading meetings |


---

## Future Goals

- Ship the remaining hardware projects (DeepThought, NFC card, bracelets)
- Build a portfolio website
- Build a mobile app with React Native
- Explore 3D modeling in Blender
- Write college essays
- Get 1500+ on SAT


---

**Last Updated:** August 2026
